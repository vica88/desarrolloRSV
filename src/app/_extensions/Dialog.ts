...
LeafletDialog.Control.Dialog = L.Control.extend({
  options: {
    size: [ 300, 300 ],
    minSize: [ 100, 100 ],
    maxSize: [ 350, 350 ],
  ...
  initialize: function(options) {
    ...
    LeafletDialog.setOptions(this, options);
  ...
  _initLayout: function() {
    var className = "leaflet-control-dialog",
      container = (this._container = LeafletDialog.DomUtil.create("div", className));
    ...
    var stop = LeafletDialog.DomEvent.stopPropagation;
    LeafletDialog.DomEvent.on(container, "click", stop)
    ...
    var innerContainer = (this._innerContainer = LeafletDialog.DomUtil.create(
      "div",
      className + "-inner"
    ));
    ...
    var grabberIcon = LeafletDialog.DomUtil.create("i", "fa fa-arrows");
    grabberNode.appendChild(grabberIcon);

    LeafletDialog.DomEvent.on(grabberNode, "mousedown", this._handleMoveStart, this);

    var closeNode = (this._closeNode = L.DomUtil.create(
      "div",
      className + "-close"
    ));
    var closeIcon = LeafletDialog.DomUtil.create("i", "fa fa-times");
    closeNode.appendChild(closeIcon);
    LeafletDialog.DomEvent.on(closeNode, "click", this._handleClose, this);

    var resizerNode = (this._resizerNode = LeafletDialog.DomUtil.create(
      "div",
      className + "-resizer"
    ));
    var resizeIcon = LeafletDialog.DomUtil.create("i", "fa fa-arrows-h fa-rotate-45");
    resizerNode.appendChild(resizeIcon);

    LeafletDialog.DomEvent.on(resizerNode, "mousedown", this._handleResizeStart, this);

    var contentNode = (this._contentNode = LeafletDialog.DomUtil.create(
      "div",
      className + "-contents"
    ));
  ...
  _handleResizeStart: function(e) {
    ...
    LeafletDialog.DomEvent.on(this._map, "mousemove", this._handleMouseMove, this);
    LeafletDialog.DomEvent.on(this._map, "mouseup", this._handleMouseUp, this);
  ...
  _handleMouseUp: function() {
    LeafletDialog.DomEvent.off(this._map, "mousemove", this._handleMouseMove, this);
    LeafletDialog.DomEvent.off(this._map, "mouseup", this._handleMouseUp, this);
...
LeafletDialog.control.dialog = function(options) {
  return new LeafletDialog.Control.Dialog(options);
};
